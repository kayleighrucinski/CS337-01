...
class NJSchoolsModel {
    ...
    //  update method for rating
        func updateSchoolsRating(forSchoolId id: Int, county: String, rating newRating: Int?) -> Bool {
            
            let countyCapitalized = county.capitalized
            
            if let index = (njCountiesNschools[countyCapitalized])?.firstIndex(where: {$0.properties.objectId == id}) {
                njCountiesNschools[countyCapitalized]?[index].properties.ratings = newRating ?? 0
                return true
            }
            return false
        }
